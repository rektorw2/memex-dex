 ...
 * Версия правил.
 *
 * Поднята со единицы до двойки при переходе на состояния проверок.
 * Воркер выбирает находки с версией меньше текущей, поэтому подъём
 * номера — и есть безопасный пересчёт: пакетами, по свежим вперёд,
 * с бюджетом времени и без изменения схемы.
 *
 * Записи, сделанные старым алгоритмом, до пересчёта не показываются
 * низкорисковыми: разбор кодов не находит вердикта полноты и относит
 * их к непроверенным. Ни `firstSeenAt`, ни сама запись при этом
 * не меняются — обновляется только оценка, и повторный проход даёт
 * тот же результат.
 */
export const RADAR_RULES_VERSION = 2;
