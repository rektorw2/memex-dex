  ...
  for (const chain of supportedChains()) {
    ...
    for (const pool of candidates) {
      ...
      const data = {
        ...
        // Возраст рынка. Провайдер отдавал его и раньше — импортёр
        // читал время пула, считал по нему `ageHours` и выбрасывал,
        // а список «Новые» тем временем сортировался по времени
        // импорта и показывал старые токены.
        //
        // Записывается только когда пришло: перетереть известное
        // время пула неизвестностью значит потерять единственный
        // точный ответ о возрасте.
        ...(pool.poolCreatedAt != null ? { poolCreatedAt: pool.poolCreatedAt } : {}),
        ...
        if (existing) {
          ...
          await prisma.token.create({
            data: {
              ...
              source: 'auto',
              isVerified: false,
              // Первое наблюдение ставится только при создании:
              // это «когда мы впервые увидели рынок», и обновлять
              // его при каждом проходе значило бы делать любой токен
              // вечно свежим.
