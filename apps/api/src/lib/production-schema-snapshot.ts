...
export async function readProductionSchemaSnapshot(
  ...
  const [
    ...
    query(COLUMNS_OF, ['PaperAgentControl']),
    query(COLUMNS_OF, ['PaperAgentStrategy']),
    query(COLUMNS_OF, ['PaperAgentRun']),
    query(COLUMNS_OF, ['PaperAgentNotification']),
    query(COLUMNS_OF, ['PaperAgentAllocationPolicy']),
    query(COLUMNS_OF, ['PaperAgentAccountSession']),
    query(COLUMNS_OF, ['PaperAgentAllocation']),
    query(COLUMNS_OF, ['PaperAgentCapitalLedger']),
  ...
  return {
    ...
    paperAgentControlColumns: names(paperAgentControlColumns),
    paperAgentStrategyColumns: names(paperAgentStrategyColumns),
    paperAgentRunColumns: names(paperAgentRunColumns),
    paperAgentNotificationColumns: names(paperAgentNotificationColumns),
    paperAgentAllocationPolicyColumns: names(paperAgentAllocationPolicyColumns),
    paperAgentAccountSessionColumns: names(paperAgentAccountSessionColumns),
    paperAgentAllocationColumns: names(paperAgentAllocationColumns),
    paperAgentCapitalLedgerColumns: names(paperAgentCapitalLedgerColumns),
