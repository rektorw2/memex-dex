...
export class ApiError extends Error {
  ...
  /**
   * Машинный код причины, если сервер его прислал.
   *
   * Контракт не меняется: код и раньше приезжал в `body`, и коммент
   * выше это описывает. Геттер добавлен затем, чтобы места вызова
   * не лазили в нетипизированный объект — сравнение по коду вместо
   * сравнения по тексту ошибки должно быть удобнее, а не наоборот.
   * Текст меняется при первой же правке формулировки, код — нет.
   */
  get code(): string | null {
    const value = this.body?.code;
