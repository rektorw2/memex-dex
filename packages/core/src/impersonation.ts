/**
 * ВНИМАНИЕ: проверка подделок переехала в token-registry.ts.
 *
 * Здесь она определялась по тикеру, и это было неверно: блокировался
 * любой токен с символом NVDA, включая настоящий, если бы он появился.
 * Правильный признак — «тикер известный, но адрес не подтверждён», и он
 * требует реестра адресов. Из этого файла осталась только checkSanity:
 * проверка правдоподобности чисел от реестра не зависит.
 *
 * Прежнее описание для истории:
 *
