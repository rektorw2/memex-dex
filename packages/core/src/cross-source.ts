...
/**
 * Расхождение цены, выше которого числам нельзя верить.
 *
...
export function crossCheck(
  ...
  const priceSpread = spread(present.map((r) => r.priceUsd));
  const liquiditySpread = spread(present.map((r) => r.liquidityUsd));
