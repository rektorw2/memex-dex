...
export function crossCheck(
  ...
  if (known === 0) {
    ...
    if (failed > 0) {
      ...
      /*
       * Отрицательный поиск — не доказательство вредоносности.
       *
       * Боевой аудит показал системный случай: импортёр получает токен
       * из OKX hot-list, а два detail-маршрута того же провайдера и
       * DexScreener отвечают пусто. Значит «двое не нашли» доказывает
       * только рассинхронизацию индексов. Версия 9 превращала её в
       * SOURCE_PRICE_MISMATCH и заблокировала почти всю витрину.
       *
       * Настоящее расхождение чисел по-прежнему блокируется выше.
       * Здесь чисел нет вовсе, поэтому остаётся честное замечание,
       * а решение принимают проверки контракта и ликвидности.
       */
      const names = considered.map((r) => r.source).join(', ');
      warnings.push(
        queried > 1
          ? `Токен не найден в detail-ответах ${names} — индексы источников могут отставать`
          : `Токен не знает единственный опрошенный источник (${names || 'нет источников'}) — этого мало для вывода`,
      );
    }
