...
describe('письмо с кодом', () => {
  ...
  it('кода в теме нет', () => {
    /*
     * Решение изменилось, и вместе с ним этот тест.
     *
     * Раньше код стоял в теме: почтовые клиенты показывают её
     * в списке, и код можно было ввести, не открывая письмо.
     * Удобство настоящее — но оно же означает, что код видит
     * каждый, кто окажется рядом с заблокированным экраном или
     * увидит список входящих через плечо.
     *
     * Тема осталась узнаваемой: продукт назван, назначение
     * письма понятно.
     */
    expect(mail.subject).not.toContain('482913');
    expect(mail.subject).toContain('Memex DEX');
    expect(mail.subject).toMatch(/подтверждени/i);
  });
