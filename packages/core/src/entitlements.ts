...
const PLAN_CAPABILITIES: Record<PlanCode, ReadonlySet<Capability>> = {
   ...
   * Пробный период.
   *
   * Даёт увидеть продукт целиком в той части, которая работает руками:
   * находки, разбор, покупка. Всё, что действует за человека —
   * копирование и автоматика, — остаётся закрытым: показать
