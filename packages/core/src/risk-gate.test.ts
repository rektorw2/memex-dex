...
describe('полная блокировка', () => {
  ...
  it('метка isSus от Jupiter прячет токен, но не блокирует', () => {
    // Проверка появилась после того, как понижение веса со 100 до 45
    // не дало ничего: код остался в CRITICAL_CODES, где вес не значит
    // ничего. Метка стояла у 54% заблокированных — столько
    // подтверждённого мошенничества не бывает.
    const res = checked([r('JUPITER_SUSPICIOUS', 45)]);
    expect(res.level).toBe('medium');
    expect(isListable(res.level, true)).toBe(false);
    expect(isListable(res.level, false)).toBe(true);
  });

  it('вес и критичность — независимые свойства кода', () => {
    // Формулировка правила, которое я нарушил. Код с весом 100,
    // не входящий в критические, даёт high; код в критических даёт
    // blocked при любом весе. Путать эти два рычага нельзя.
    expect(checked([r('SUSPICIOUS_VOLUME', 100)]).level).toBe('high');
    expect(checked([r('HONEYPOT', 1)]).level).toBe('blocked');
  });
